...
<?php if (!empty($do_an_list)): ?>
    <?php foreach ($do_an_list as $index => $item): ?>
        <tr>
            ...
            <td><?php echo htmlspecialchars($item['ten_de_tai']); ?></td>
            <td>
                <?php echo htmlspecialchars($item['ten_sinh_vien']); ?>
                (<?php echo htmlspecialchars($item['mssv']); ?>)
            </td>
            <td><?php echo htmlspecialchars($item['giang_vien_hd']); ?></td>
            <td><?php echo htmlspecialchars($item['nam_hoc']); ?></td>
...
<?php else: ?>
    <tr>
        <td colspan="7" style="text-align:center; color:#666; padding:16px;">
            Chưa có đồ án nào trong mảng.
        </td>
    </tr>
<?php endif; ?>
</tbody>
